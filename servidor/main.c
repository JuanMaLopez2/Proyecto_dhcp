...
unsigned char direccionMacCliente[16];  
uint32_t idTransaccionCliente;  
int direccionIpDisponible = 1;  // Inicialmente, tenemos una IP disponible para asignar
...
int main() {
    ...
    while (1) {
        ...
        if (recibirDhcpDiscover(socketFd, &cliente, &longitudDir, buffer)) {
